...
for (const [dirIndex, pencilDirectory] of pencilDirectories.entries()) {
	...
	if (pencilFiles.length > 0) {
		...
		for (const [index, pencilFile] of pencilFiles.entries()) {
			...
			fileNumber = await renderSVGAndPNG(new SVGTechnicalRenderer(pencil, pencilDirectory), -1, "technical", pencilDirectory, pencilFileName, fileNumber);
			fileNumber = await renderSVGAndPNG(new SVGTechnicalGroupedRenderer(pencil, pencilDirectory), -1, "technical", pencilDirectory, pencilFileName + "-grouped", fileNumber);
			fileNumber = await renderSVGAndPNG(new SVGTechnicalComponentRenderer(pencil, pencilDirectory), -1, "technical", pencilDirectory, pencilFileName + "-components", fileNumber);
			fileNumber = await renderSVGAndPNG(new SVGTechnicalExplodedRenderer(pencil, pencilDirectory), -1, "technical", pencilDirectory, pencilFileName + "-exploded", fileNumber);

			fileNumber = await renderSVGAndPNG(new SVGPencilRenderer(pencil, pencilDirectory), -1, "pencil", pencilDirectory, pencilFileName, fileNumber);
			fileNumber = await renderSVGAndPNG(new SVGPencilAllRenderer(pencil, pencilDirectory), -1, "pencil", pencilDirectory, pencilFileName + "-all-variants", fileNumber);
			fileNumber = await renderSVGAndPNG(new SVGPencil45Renderer(pencil, pencilDirectory), -1, "pencil", pencilDirectory, pencilFileName + "-45", fileNumber);
			...
			for(let [ index, colourComponent ] of pencil.colourComponents.entries()) {
				...
				fileNumber = await renderSVGAndPNG(new SVGTechnicalRenderer(pencil, pencilDirectory), index, "technical", pencilDirectory, pencilColourOutputFileName, fileNumber);
				fileNumber = await renderSVGAndPNG(new SVGTechnicalExplodedRenderer(pencil, pencilDirectory), index, "technical", pencilDirectory, pencilExplodedColourOutputFileName, fileNumber);
				fileNumber = await renderSVGAndPNG(new SVGPencil45Renderer(pencil, pencilDirectory), index, "pencil", pencilDirectory, pencilColourOutputFileName + "-45", fileNumber);
				fileNumber = await renderSVGAndPNG(new SVGPencilRenderer(pencil, pencilDirectory), index, "pencil", pencilDirectory, pencilColourOutputFileName, fileNumber);
				numColours++;
			}
			//
