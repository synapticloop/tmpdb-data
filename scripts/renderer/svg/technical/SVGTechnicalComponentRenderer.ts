...
export class SVGTechnicalComponentRenderer extends SVGRenderer {
	...
	render(colourIndex: number):string {
		...
		svgString += super.renderCentreLineHorizontal(this._height/2);
	...
	private renderGuidelinesFull(): string {
		...
		svgString += lineVerticalGuide(offset, this._height/2 - 88 - this.pencil.maxHeight/2 * 5, 88 + this.pencil.maxHeight/2 * 5);
