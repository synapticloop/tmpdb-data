 ...
 */
export class SVGExampleWithFrontRenderer extends SVGRenderer {
	...
	render(colourIndex: number):string {
		...
		for(const component of this.pencil.components){
			...
			svgString += drawTextBoldCentred(component.type, this._width/2 - component.length/2 * 5 - 120, midY - 60, "1.0em");
			svgString += super.renderFrontComponent(this._width/2 - component.length/2 * 5 - 120, midY, component, colourIndex);
