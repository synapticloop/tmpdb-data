...
import { circle, dimensionsHorizontal, dimensionsVertical, drawOutlineCircle, drawOutlineHexagon, drawOutlineOctagon, lineHorizontalGuide, lineVerticalGuide, TextOrientation } from "../utils/svg-helper.ts";
...
export class SVGTechnicalRenderer extends SVGRenderer {
    ...
    renderGuidelinesFull() {
        ...
        for (let component of this.pencil.components) {
            ...
            // now for extraParts
            for (const extra of component.extras) {
                svgString += lineVerticalGuide(offset + extra.offset[0] * 5, this.SVG_HEIGHT / 2 - 80, 160);
                svgString += lineVerticalGuide(offset + extra.offset[0] * 5 + extra.length * 5, this.SVG_HEIGHT / 2 - 80, 160);
            }
    ...
    renderFrontDimensions() {
        ...
        if (thisExtraPart) {
            ...
            svgString += dimensionsHorizontal(160 - thisExtraPart.width / 2 * 5, this.SVG_HEIGHT / 2 - 60, thisExtraPart.width * 5, `${(Math.round((thisExtraPart.width) * 100) / 100).toFixed(2)} mm`, TextOrientation.TOP, true);
            svgString += dimensionsVertical(222, this.SVG_HEIGHT / 2 - extrasOffset * 5 - extrasHeight * 5, extrasHeight * 5, `${(Math.round((thisExtraPart.height) * 100) / 100).toFixed(2)} mm`, TextOrientation.RIGHT);
            svgString += dimensionsVertical(210, this.SVG_HEIGHT / 2 - this.pencil.maxHeight / 2 * 5, this.pencil.maxHeight * 5, `${(Math.round((this.pencil.maxHeight) * 100) / 100).toFixed(2)} mm`, TextOrientation.RIGHT);
            ...
            svgString += dimensionsVertical(110, this.SVG_HEIGHT / 2 - extrasOffset * 5 - extrasHeight * 5, pencilExtraHeight * 5, `${pencilExtraHeight} mm`, TextOrientation.LEFT_ROTATED, true);
        }
        else {
            // no extra height here - so we can just draw the text
            svgString += dimensionsVertical(110, this.SVG_HEIGHT / 2 - this.pencil.maxHeight / 2 * 5, this.pencil.maxHeight * 5, `${(Math.round((this.pencil.maxHeight) * 100) / 100).toFixed(2)} mm`, TextOrientation.LEFT_ROTATED, true);
        }
        // This is the BOTTOM WIDTH of the pencil
        svgString += dimensionsHorizontal(160 - this.pencil.maxWidth / 2 * 5, this.SVG_HEIGHT / 2 + 30 + this.pencil.maxHeight / 2 * 5, this.pencil.maxWidth * 5, `${(Math.round(this.pencil.maxWidth * 100) / 100).toFixed(2)} mm`, TextOrientation.BOTTOM, true);
        return (svgString);
    }
    renderBackDimensions() {
        ...
        for (let component of this.pencil.components) {
            if (component.type === "body") {
                svgString += dimensionsVertical(this.SVG_WIDTH - 150, this.SVG_HEIGHT / 2 - component.maxHeight / 2 * 5, component.maxHeight * 5, `${component.maxHeight} mm`, TextOrientation.LEFT_ROTATED, true);
                svgString += dimensionsVertical(this.SVG_WIDTH - 150, this.SVG_HEIGHT / 2 - component.maxHeight / 2 * 5, component.maxHeight * 5, "body", TextOrientation.BOTTOM_ROTATED, true);
            }
            if (component.type === "grip") {
                svgString += dimensionsVertical(this.SVG_WIDTH - 190, this.SVG_HEIGHT / 2 - component.maxHeight / 2 * 5, component.maxHeight * 5, `${component.maxHeight} mm`, TextOrientation.LEFT_ROTATED, true);
                svgString += dimensionsVertical(this.SVG_WIDTH - 190, this.SVG_HEIGHT / 2 - component.maxHeight / 2 * 5, component.maxHeight * 5, "grip", TextOrientation.BOTTOM_ROTATED, true);
            }
        }
        return (svgString);
    }
    renderFrontComponents(colourIndex) {
        ...
        let colour = new OpacityColour(this.pencil.colourMap, "white%0");
        ...
        for (const component of this.pencil.components) {
            ...
            for (let part of component.parts) {
                ...
                switch (part.shape) {
                    case "cylinder":
                        svgString += circle(startX, midY, (part.startHeight / 2) * 5, "0.5", "black", colour);
                        break;
                    case "cone":
        ...
        for (let front of this.pencil.front) {
            ...
            let frontFillColour = new OpacityColour(this.pencil.colourMap, "white");
            if (colourIndex !== -1) {
    ...
    renderBackComponents(colourIndex) {
        ...
        let midY = this.SVG_HEIGHT / 2;
        let colour = new OpacityColour(this.pencil.colourMap, "white");
        // go through the components and render them
        for (const component of this.pencil.components) {
        ...
        for (let back of this.pencil.back) {
            let backFillColour = new OpacityColour(this.pencil.colourMap, "white");
            if (colourIndex !== -1) {
                // TODO mapped colour
