...
var FontFamily;
(function (FontFamily) {
    FontFamily[FontFamily["HEADING_LARGE"] = 0] = "HEADING_LARGE";
    FontFamily[FontFamily["HEADING_MEDIUM"] = 1] = "HEADING_MEDIUM";
    FontFamily[FontFamily["HEADING_SMALL"] = 2] = "HEADING_SMALL";
    FontFamily[FontFamily["HEADING_TINY"] = 3] = "HEADING_TINY";
    FontFamily[FontFamily["PARAGRAPH"] = 4] = "PARAGRAPH";
    FontFamily[FontFamily["FOOTER_COPYRIGHT"] = 5] = "FOOTER_COPYRIGHT";
...
export class PDFDatasheetRenderer {
    ...
    pageTitles = [];
    currentPageTitle;
    constructor(pencil, pencilFileDirectory, pencilFileName) {
        this.pencil = pencil;
    ...
    renderFrontPage(doc) {
        ...
        doc.moveTo(doc.page.margins.left, doc.y)
            ...
            .stroke("#000000");
        doc.text("").moveDown(1);
        this.centreImage(doc, `./output/png/pencil/${this.pencilFileDirectory}/${this.pencilFileName}-all-variants.png`, 450);
        this.addPageWithTitle(doc, "Datasheet");
        this.setFontFamily(doc, FontFamily.PARAGRAPH);
        doc.text('').moveDown();
        this.centreImage(doc, `./output/png/technical/${this.pencilFileDirectory}/${this.pencilFileName}.png`, 500);
        doc.text("").moveDown(2);
    ...
    renderExternalMeasurementTable(doc) {
    ...
    renderExternalMaterialsTable(doc) {
        this.setFontFamily(doc, FontFamily.HEADING_SMALL);
        doc.text("").moveDown(1);
    ...
    renderColourVariantsPage(doc) {
        ...
        for (const [index, colourComponent] of this.pencil.colourComponents.entries()) {
            ...
            this.centreImage(doc, `./output/png/pencil/${this.pencilFileDirectory}/${this.pencilFileName}-colour-${colourComponent.colourName}.png`, 400);
            doc.text('').moveDown(1);
            this.setFontFamily(doc, FontFamily.PARAGRAPH);
            const componentData = [];
            ...
            colourData.push("");
            const numColumns = colourData.length;
            const colourNameData = [];
    ...
    addPageWithTitle(doc, pageTitle) {
    ...
    addHeader(doc, pageNumber) {
        ...
        if (pageNumber === 0) {
            ...
            doc.text('').moveUp(2);
            const currentTitle = this.pageTitles[pageNumber];
            const previousTitle = this.pageTitles[pageNumber - 1];
    ...
    addFooter(pageNumber, doc) {
        ...
        this.setFontFamily(doc, FontFamily.HEADING_TINY);
        doc.fillColor("black")
            .text('Page ' + pageNumber, 0.5 * (doc.page.width - 100), footerStartY + 10, {
            width: 100,
            align: 'center',
            lineBreak: false,
        });
        ...
        doc.text('').moveDown(1);
        doc.fillColor("black")
            .text("Copyright (c) // The Mechanical Pencil Database (tmpdb)", doc.page.margins.left, doc.y, { align: "center" });
        doc.fillColor("black")
            .text("Licensed under Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International", doc.page.margins.left, doc.y, { align: "center" });
        doc.page.margins.bottom = bottom;
        doc.fillColor("black")
            .text('', doc.page.margins.left, doc.page.margins.top);
    }
    setFontFamily(pdfDocument, fontFamily) {
    ...
    centreImage(pdfDocument, imageLocation, width) {
        const buffer = fs.readFileSync(imageLocation);
        const dimensions = imageSize(buffer);
        const imageWidth = dimensions.width;
        const imageHeight = dimensions.height;
        const drawableWidth = pdfDocument.page.width - pdfDocument.page.margins.left - pdfDocument.page.margins.right;
        const moveDown = width / imageWidth * imageHeight;
        let writablePageHeight = pdfDocument.page.height - pdfDocument.page.margins.top - pdfDocument.page.margins.bottom;
        ...
        // 			{width: width});
        // }
        pdfDocument.image(imageLocation, pdfDocument.x + ((drawableWidth - width) / 2), pdfDocument.y, { width: width });
        pdfDocument.y = pdfDocument.y + moveDown;
    }
    appendHeaderAndFooter(pdfDocument) {
    ...
    getFinish(parts) {
        ...
        let textArray = [];
        for (const part of parts) {
            const finishes = part.finish.split(",");
...
//# sourceMappingURL=PDFDatasheetRenderer.js.map