...
import { drawText, lineHorizontal, lineHorizontalGuide, lineVertical, lineVerticalGuide } from "../utils/svg-helper.ts";
...
export class SVGExplodedTechnicalRenderer extends SVGRenderer {
    ...
    renderGuidelines() {
        ...
        for (let component of this.pencil.components) {
            ...
            // now for extraParts
            for (const extraPart of component.getExtraParts()) {
                svgString += lineVerticalGuide(offset + extraPart.extraOffset[0] * 5 - (component.extraPartFirst ? component.length * 5 : 0), this.SVG_HEIGHT / 2 - 80, 160);
                svgString += lineVerticalGuide(offset + extraPart.extraOffset[0] * 5 + extraPart.extraLength * 5 - (component.extraPartFirst ? component.length * 5 : 0), this.SVG_HEIGHT / 2 - 80, 160);
            }
        }
    ...
    renderSideComponents(colourIndex) {
        let svgString = "";
        let startX = this.SVG_WIDTH / 2 - (this.pencil.totalLength * 5 / 2);
        let midY = 120;
        let colour = "white";
        for (let [index, component] of this.pencil.components.entries()) {
            ...
            if (component.hasInternal) {
                ...
                let totalLength = component.length;
                for (let internalPart of component.internals) {
                ...
                component.internals.reverse();
                startX += partLength;
                svgString += drawText(`${component.type} (${formatToTwoPlaces(totalLength)} mm)`, startX + 50, midY, "1.1em");
            }
        }
...
//# sourceMappingURL=SVGExplodedTechnicalRenderer.js.map