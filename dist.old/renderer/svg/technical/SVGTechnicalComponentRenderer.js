...
export class SVGTechnicalComponentRenderer extends SVGRenderer {
     ...
     */
    render(colourIndex) {
        let componentOffset = 300;
        let hiddenOffset = 500;
        let xMid = this._width / 2;
    ...
    renderGuidelinesFull(x, y) {
        ...
        for (let component of this.pencil.components) {
            ...
            for (const extra of component.extras) {
                svgString += lineVerticalGuide(x + extra.xOffset * 5, y - 80, 80);
                svgString += lineVerticalGuide(x + extra.xOffset * 5 + extra.length * 5, y - 80, 80);
            }
