...
export class SVGRenderer {
    static inBuiltPatternMap = new Map();
    ...
    renderTaper(startX, midY, part, colourIndex, colour) {
        ...
        if (part.taperEnd) {
            ...
            switch (part.shape) {
                case "hexagonal":
                    svgString += rectangle(startX - xOffsetTaperEnd * 5, midY - part.endHeight / 2 * 5 + 0.25, (part.length + xOffsetTaperEnd) * 5 - 0.5, part.startHeight * 5 - 0.5, "none", backgroundColour);
    ...
    renderFrontDimensions(x, y) {
        ...
        if (thisExtraPart) {
            ...
            svgString += dimensionsHorizontal(x - thisExtraPart.depth / 2 * 5, y - 60, thisExtraPart.depth * 5, `${formatToTwoPlaces(thisExtraPart.depth)} mm`, TextOrientation.TOP, true);
    ...
    renderSideDimensions(x, y) {
        ...
        for (let component of this.pencil.components) {
            ...
            svgString += dimensionsHorizontal(x, y - 120, component.length * 5, `${formatToTwoPlaces(component.length)} mm${(component.length * 5 > 30 ? "\n" : " ")}${component.type}`, TextOrientation.TOP_ROTATED, true);
            // now for the extra dimensions
            // is the extra the first component, or the last
            // now for extraParts
            for (const extra of component.extras) {
                // draw the straight-through line for guidance
                svgString += dimensionsHorizontal(x + extra.xOffset * 5, y - 80, extra.length * 5, `${formatToTwoPlaces(extra.length)} mm\n${component.type} (extra)`, TextOrientation.CENTER, true);
            }
    ...
    renderSideMaterials() {
        ...
        for (let component of this.pencil.components) {
            ...
            for (const extra of component.extras) {
                svgString += dimensionsHorizontal(xOffset + extra.xOffset * 5, this._height / 2 + 80, extra.length * 5, `${component.materials.join("\n")}`, TextOrientation.BOTTOM, false);
            }
            // draw all the dimensions
    ...
    renderSideComponents(x, y, colourIndex) {
        let svgString = "";
    ...
    renderHiddenSideComponents(x, y, colourIndex) {
        let svgString = "";
        let colourOpacity = new OpaqueColour(this.pencil.colourMap, "white");
        for (let component of this.pencil.components) {
            if (component.isHidden) {
    ...
    /**
     * Render a specific part, with any extras that are attached to it, the order
     * of rendering for a part is as follows:
     *
     *  - The background for any extra parts
     ...
     *    finishes
     *
     * @param x The x co-ordinate at which to start the drawinf of the part
     * @param y The y co-ordinate at which to start the drawing of trhe part
     * @param component The component (for the extras)
     * @param part The part to render
     * @param colourIndex The colour Index for the pencil colour
     * @param defaultOpaqueColour // TODO - delete me
     *
     * @protected
     */
    renderPart(x, y, component, part, colourIndex) {
        // TODO - we are drawing multiple extras - this should be done before/after
        //  rendering the part
        let svgString = "";
        ...
        // if any of the extras are black - outline in dimgray
        for (const extra of component.extras) {
        ...
        switch (part.shape) {
            ...
            case "cone":
                ...
                // TODO - move to later....
                if (part.joined) {
                    svgString += lineJoined(x, y - (part.endHeight / 2 * 5) + 0.25, part.startHeight * 5 - 0.5, "3", opaqueColour);
        ...
        let taperStartOffset = (part.taperStart?.xOffset ? part.taperStart.xOffset : 0);
        let taperEndOffset = (part.taperEnd?.xOffset ? part.taperEnd.xOffset : 0);
        switch (part.shape) {
            case "hexagonal":
        ...
        for (const finish of part.finish.split(",")) {
            let xStart = x + (part.internalOffset) * 5;
            let xEnd = x + part.length * 5;
            let yStartTop = y - (part.startHeight / 2 * 5);
            let yStartBottom = y + (part.startHeight / 2 * 5);
            let yEndTop = y - (part.endHeight / 2 * 5);
            let yEndBottom = y + (part.endHeight / 2 * 5);
        ...
        for (const extra of component.extras) {
            ...
            svgString += drawExtraForeground(x + extra.xOffset * 5, y - extra.yOffset * 5, extra.extraParts, colour.colour);
            break;
        }
        return (svgString);
    }
    renderGuidelines() {
        ...
        for (let component of this.pencil.components) {
            ...
            // now for extraParts
            for (const extra of component.extras) {
                svgString += lineVerticalGuide(offset + extra.xOffset * 5, this._height / 2 - 80, 80);
                svgString += lineVerticalGuide(offset + extra.xOffset * 5 + extra.length * 5, this._height / 2 - 80, 80);
            }
        }
        svgString += lineVerticalGuide(offset, this._height / 2 - 88 - this.pencil.maxHeight / 2 * 5, 140 + this.pencil.maxHeight / 2 * 5);
        return (svgString);
