...
export function drawShapeDetails(x, y, width) {
    return (`<line x1="${x}" ` +
        `y1="${y}" ` +
        `x2="${x + width}" ` +
        `y2="${y}" ` +
...
export function drawExtraBackground(offsetX, offsetY, extraParts, strokeColour) {
    let svgString = "";
    for (const extraPart of extraParts) {
        const points = extraPart.points;
...
export function drawExtraForeground(offsetX, offsetY, extraParts, fillColour) {
    let svgString = "";
    for (const extraPart of extraParts) {
        const points = extraPart.points;
        switch (extraPart.shape) {
...
export function renderExtra(x, y, offsetX, offsetY, width, parts, fillColour) {
    let svgString = "";
    let points = [];
    let isCurve = false;
    for (const part of parts) {
...
export function lineHorizontalGuide(x, y, width) {
    return (lineHorizontal(x, y, width, "1", "#cfcfcf"));
}
export function lineHorizontal(x, y, width, strokeWidth, strokeColour, strokeDash = null) {
    let svgString = `<line x1="${x}" ` +
        `y1="${y}" ` +
        `x2="${x + width}" ` +
        `y2="${y}" ` +
...
export function lineVerticalGuide(x, y, height, strokeWidth = "1") {
    return (lineVertical(x, y, height, strokeWidth, "#cfcfcf"));
}
export function circle(x, y, radius, strokeWidth, strokeColour, fillColour = new OpaqueColour(null, "white%0")) {
    return (`<circle r="${radius}" ` +
        `cx="${x}" ` +
        `cy="${y}" ` +
...
export function lineVertical(x, y, height, strokeWidth, strokeColour, strokeDashArray = null) {
    let svgString = "";
    svgString += `<line x1="${x}" ` +
        `y1="${y}" ` +
        `x2="${x}" ` +
...
export function lineJoined(x, y, height, strokeWidth, strokeColour) {
    let svgString = "";
    svgString += `<line x1="${x}" ` +
        `y1="${y}" ` +
        `x2="${x}" ` +
...
export function target(x, y, length, radius) {
    let svgString = "";
 ...
 */
export function dimensionsVertical(x, y, height, text = null, textOrientation = TextOrientation.TOP, shouldBold = true) {
    let svgString = "";
 ...
 * @param shouldBold Optional, whether the text should be bold (default 'true')
 */
export function dimensionsHorizontal(x, y, width, text = null, textOrientation = TextOrientation.TOP, shouldBold = true) {
    let svgString = "";
...
export function rectangle(x, y, width, height, strokeColour, opaqueColour) {
    let svgString = "";
    svgString += `<rect x="${x}" ` +
        `y="${y}" ` +
        `width="${width}" ` +
