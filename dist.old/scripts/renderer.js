...
for (const pencilDirectory of pencilDirectories) {
    ...
    if (pencilFiles.length > 0) {
        ...
        for (const [index, pencilFile] of pencilFiles.entries()) {
            ...
            const pdfOutputDir = path.join("./output/pdf/datasheet/");
            fs.mkdirSync(pdfOutputDir, { "recursive": true });
            const outputPdfFie = path.join(pdfOutputDir, pencilDirectory + "-" + pencilFileName + ".pdf");
            ...
            fileNumber++;
            const scadOutputDir = path.join("./output/scad/technical/");
            fs.mkdirSync(scadOutputDir, { "recursive": true });
            const outputScadFile = path.join(scadOutputDir, pencilDirectory + "-" + pencilFileName + ".scad");
            const outputScadString = new OpenSCADRenderer(pencil).render();
