...
import { circle, dimensionsHorizontal, drawExtra, drawOutlineCircle, drawOutlineHexagon, drawOutlineOctagon, drawShapeDetails, drawTextBoldCentred, lineHorizontal, lineVertical, lineVerticalGuide, renderBackExtra, target, TextOrientation } from "../utils/svg-helper.ts";
export class SVGTechnicalComponentRenderer extends SVGRenderer {
    static SVG_WIDTH = 1200;
    static SVG_HEIGHT = 500;
    ...
    render(colourIndex) {
        // start
        let svgString = SVGTechnicalComponentRenderer.SVG_START;
        // centre line
    ...
    renderOverviewText() {
        ...
        svgString += drawTextBoldCentred(`Components for ${this.pencil.brand}` +
            ` // ` +
            `${this.pencil.model} ` +
            `${(this.pencil.modelNumber ? "(Model #: " + this.pencil.modelNumber + ")" : "")}`, SVGTechnicalComponentRenderer.SVG_WIDTH / 2, SVGTechnicalComponentRenderer.SVG_HEIGHT - 90, "2.0em");
        return (svgString);
    }
    renderGuidelines() {
        let svgString = "";
        ...
        for (let component of this.pencil.components) {
            ...
            for (const extraPart of component.getExtraParts()) {
                svgString += lineVerticalGuide(offset + extraPart.extraOffset[0] * 5 - (component.extraPartFirst ? component.length * 5 : 0), SVGTechnicalComponentRenderer.SVG_HEIGHT / 2 - 80, 80);
                svgString += lineVerticalGuide(offset + extraPart.extraOffset[0] * 5 + extraPart.extraLength * 5 - (component.extraPartFirst ? component.length * 5 : 0), SVGTechnicalComponentRenderer.SVG_HEIGHT / 2 - 80, 80);
            }
        }
        svgString += lineVerticalGuide(offset, SVGTechnicalComponentRenderer.SVG_HEIGHT / 2 - 88 - this.pencil.maxHeight / 2 * 5, 140 + this.pencil.maxHeight / 2 * 5);
        return (svgString);
    ...
    renderSideDimensions() {
        let svgString = "";
        let xOffset = SVGTechnicalComponentRenderer.SVG_WIDTH / 2 - (this.pencil.totalLength * 5 / 2);
        for (let component of this.pencil.components) {
            // draw all the dimensions
            svgString += dimensionsHorizontal(xOffset, SVGTechnicalComponentRenderer.SVG_HEIGHT / 2 - 120, component.length * 5, `${component.type}`, TextOrientation.TOP_ROTATED, true);
            ...
            for (const extraPart of component.getExtraParts()) {
                ...
                svgString += dimensionsHorizontal(xOffset + extraPart.extraOffset[0] * 5 - (component.extraPartFirst ? component.length * 5 : 0), SVGTechnicalComponentRenderer.SVG_HEIGHT / 2 - 80, extraPart.extraLength * 5 - (component.extraPartFirst ? component.length * 5 : 0), `${component.getType()} (extra)`, TextOrientation.TOP, true);
            }
        }
        // now for the total length
        svgString += dimensionsHorizontal(SVGTechnicalComponentRenderer.SVG_WIDTH / 2 - this.pencil.totalLength * 5 / 2, SVGTechnicalComponentRenderer.SVG_HEIGHT / 2 + 30 + this.pencil.maxHeight / 2 * 5, this.pencil.totalLength * 5, "", TextOrientation.BOTTOM, true);
        return (svgString);
    }
    renderSideComponents(colourIndex) {
        let svgString = "";
        let startX = SVGTechnicalComponentRenderer.SVG_WIDTH / 2 - (this.pencil.totalLength * 5 / 2);
...
//# sourceMappingURL=SVGTechnicalComponentRenderer.js.map