...
export function lineHorizontalDimension(x, y, width) {
...
export function lineVerticalGuide(x, y, height) {
    return (lineVertical(x, y, height, "1", "#cfcfcf"));
}
export function lineVerticalDimensions(x, y, height) {
    return (lineVertical(x, y, height, "1", "#000000"));
}
export function circle(x, y, radius, strokeWidth, strokeColour, fillColour = "none") {
    return (`<circle r="${radius}" ` +
        `cx="${x}" ` +
        `cy="${y}" ` +
        `fill="${fillColour}" ` +
...
export function lineVertical(x, y, height, strokeWidth, strokeColour, strokeDash = null) {
    let svgString = "";
    svgString += `<line x1="${x}" ` +
        `y1="${y}" ` +
        `x2="${x}" ` +
